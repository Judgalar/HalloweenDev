...
function battleHorde(zombies: string, humans: string) {
    ...
    const zombiesNumbers = Array.from(zombies).map(Number)
    const humansNumbers = Array.from(humans).map(Number)
    let humanPower = 0
    let zombiePower = 0

    for(let i = 0; i < rounds; i++) {
        const human = humansNumbers[i] + humanPower
        const zombie = zombiesNumbers[i] + zombiePower
        ...
        if (human == zombie) {
            ...
            humanPower = human - zombie
            zombiePower = 0
        }
        else {
            zombiePower = zombie - human
            humanPower = 0
        }
    }
